...
| 工具 | 用途 | 关键约束 |
|---|---|---|
| `plc_write_pvi` | 写入白名单测试变量 | 必须 `execute=true`，禁止生产目标 |
| `plc_run_io_test_case` | 单个输入输出测试 | 只写测试 harness 白名单 |
| `plc_run_test_suite` | 批量运行测试套件 | 输出 pass/fail 报告 |
| `plc_reset_test_harness` | 恢复测试变量安全状态 | 只写 reset 白名单 |
...
4. plc_reset_test_harness(execute=true)            → 测试前复位
5. plc_run_test_suite(execute=true)                → 写输入、读输出、断言
6. plc_reset_test_harness(execute=true)            → 测试后恢复
```
...
7. **禁止任意写 PLC 变量**。M6 写入能力只能面向 `pvi.write_whitelist` 中的测试 harness 变量。
